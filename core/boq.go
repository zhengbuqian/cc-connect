...
// TopicNameUpdater is an optional Platform interface for updating the in-memory
// topic name cache after a programmatic rename. This allows boqRemoveTopicPrefix
// to read back the name we set, even if the platform can't reliably fetch it.
type TopicNameUpdater interface {
	UpdateTopicNameCache(replyCtx any, name string)
}
